...
namespace CSWSWEB.DataAccesse
{
    public class QuoteDAL
    {
        ...
        public ArrayList getAllQuoteIDs(bool isToday = false)
        {
            string sqlCommand = "SELECT quote_id FROM quotes where deleted=0";

            if(isToday)
            {
                sqlCommand = "SELECT quote_id FROM quotes where deleted=0 and is_today=0";
            }
                ...
                using (MySqlCommand mysqlCmd = new MySqlCommand(sqlCommand, mysqlConnection))
                {
                    ...
                    if (dtReturns != null)
                    {
                        if (dtReturns.Rows.Count > 0)
                        {
                            ...
                            {
                                sList.Add(row["quote_id"].ToString());
                            }
                        }
                        else
                            sList = null;
        ...
        {
            ArrayList listQuotes = getAllQuoteIDs(true);
            int rdIndex = 0;

            int maxNum = listQuotes.Count;

            rdIndex = new Random().Next(1, maxNum);

            int rd_quote_id = Convert.ToInt32(listQuotes[rdIndex]);


           
            string sp_procedure_name = "sp_get_today_quote";
            ...
            using (MySqlConnection mysqlConnection = dbconnection.CSWSWEB_MySqlConnectIon())
            {
                using (MySqlCommand mysqlCmd = new MySqlCommand(sp_procedure_name, mysqlConnection))
                {
                    mysqlCmd.CommandType = CommandType.StoredProcedure;
                        ...
                        mysqlCmd.Parameters.AddWithValue("@random_quote_id", rd_quote_id);
        ...
        public DataTable getRandomQuote(bool isToday = false)
        {

            ArrayList listQuotes = getAllQuoteIDs(isToday);
            ...
            int quote_id = Convert.ToInt32(listQuotes[rdIndex]);


            string sqlCommand = "SELECT * FROM quotes where quote_id=@quote_id and deleted=0";
            ...
            using (MySqlConnection mysqlConnection = dbconnection.CSWSWEB_MySqlConnectIon())
            {
                using (MySqlCommand mysqlCmd = new MySqlCommand(sqlCommand, mysqlConnection))
                {
                    mysqlCmd.Parameters.AddWithValue("@quote_id", quote_id);
                    mysqlConnection.Open();
                    MySqlDataReader myReader = mysqlCmd.ExecuteReader();
        ...
        public DataTable getQuoteSelectorByCulture(string sFieldName, int countryIndex = 0)
        {
            ...
            else if (countryIndex == 2) // for quote page to search quote, only search the countries in quotes table
            {
                CSWS_Utility cswsUT= new CSWS_Utility();
                string culture = cswsUT.getCurrentCulture().ToLower();
                string culture_field = cswsUT.getCountryCultureCode(culture);
                sqlCommand = @"SELECT DISTINCT "+culture_field+@" country_culture, pub_country FROM quotes q, country_culture c
                               WHERE q.pub_country!='' and q.deleted=0 
                               and q.pub_country = c.country
                               ORDER BY pub_country ASC";
            
            }

            if (sFieldName == "author")
                sqlCommand = "SELECT  DISTINCT trim(author) author " +
            ...
            using (MySqlConnection mysqlConnection = dbconnection.CSWSWEB_MySqlConnectIon())
            {
                using (MySqlCommand mysqlCmd = new MySqlCommand(sqlCommand, mysqlConnection))
                {
                    // mysqlCmd.Parameters.AddWithValue("@fieldName", sFieldName);
                    mysqlConnection.Open();
                    MySqlDataReader myReader = mysqlCmd.ExecuteReader();
                    dataTable.Load(myReader);
                    return dataTable;
